      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {team.map((member) => (
            ...
            >
              <div className="mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-primary font-display text-xl font-bold text-primary-foreground">
                {member.name.split(" ").map((n) => n[0]).join("")}
              </div>
              <h3 className="font-display text-base font-bold text-card-foreground">
                {member.name}
              </h3>
              <p className="mt-0.5 font-body text-xs font-medium text-gold">
                {member.role}
              </p>
